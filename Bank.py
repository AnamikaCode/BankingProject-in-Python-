...
def finish_reg():
    ...
    for name_check in all_accounts:
        if name == name_check:
            ...
            new_file.write('0')
            new_file.close()
            notif.config(fg="green", text="Account will Created")
...
def deposit():
    ...
    Button(deposit_screen,text="Finish",font=('Calibri',12),command=finish_deposite).grid(row=3,sticky=W,pady=5)
def finish_deposite():
    if amount.get() == "" :
...
def withdraw():
    ...
    Button(withdraw_screen, text="Finish", font=('Calibri', 12), command=finish_withdraw).grid(row=3, sticky=N)
def finish_withdraw():
    ...
    if float(withdraw_amount.get()) >float(current_balance):
        withdraw_notif.config(text="Insufficient",fg="red")
        return
    updated_balance = current_balance
...
def login():
    ...
    Button(login_screen, text='Login',command=login_session, width=15,font=('calibri',12)).grid(row=3, sticky=N,padx=5,pady=5)
#image  import
img = Image.open("Lockup.png")
img = img.resize((450,350))
img = ImageTk.PhotoImage(img)

Label(master,text = "Custom Banking", fg="blue",font=("Calibri",18)).grid(row=0, sticky=N,pady=10)
Label(master,text="The most secure bank you've probably used", fg="black",font=("Calibri",16)).grid(row=1,sticky=N)
